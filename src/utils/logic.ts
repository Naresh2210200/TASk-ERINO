...
export function computeROI(revenue: number, timeTaken: number): number | null {
  // Validate inputs: check for invalid numbers, zero time, or negative values
  if (!Number.isFinite(revenue) || !Number.isFinite(timeTaken)) {
    return null;
  }
  if (timeTaken <= 0) {
    return null; // Return null for division by zero or invalid time
  }
  if (revenue < 0) {
    return null; // Return null for negative revenue
  }
...
export function sortTasks(tasks: ReadonlyArray<DerivedTask>): DerivedTask[] {
  return [...tasks].sort((a, b) => {
    ...
    // Stable tie-breaker: sort alphabetically by title, then by ID if titles are equal
